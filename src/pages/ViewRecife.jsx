...
const ViewRecife = () => {
  ...
  //     Swal.fire(
  //       'Good job!',
  //       'The food is added to favourite!',
  //       'success'
  //     )
  //   }
  // };
  ...
  const handleBookmarkClick = (recipeName) => {
    if (favoriteItems.includes(recipeName)) {
      ...
      const updatedFavorites = favoriteItems.filter(item => item !== recipeName);
        ...
        {chef.recife_info.map((info) => (
          <div key={info.name} className="card w-96 bg-base-100 shadow-xl">
              <div className="card-body">
                ...
                >
                  <FaBookmark
                    color={favoriteItems.includes(info.recipe_name) ? 'red' : 'inherit'}
                  />
                </button>
                </div>
              </div>
